...
- `DEALER`：可在小程序提交预约安装；不能指定师傅或修改订单。公司维度订单权限未开放，不能因提交预约而查看未直接绑定本人的订单。预约创建只返回回执，管理员可查看来源、提交经销商并修改派单。

已登录管理员可通过“用户管理 → 新增用户”创建可登录后台的管理员：选择 ADMIN 后必须填写唯一登录账号、密码和确认密码。账号统一为小写，密码以 BCrypt 哈希保存；其他角色不能设置后台登录凭据。字段、错误码和验证见 [管理员创建说明](ADMIN_USER_CREATION.md)。
